...
NSString* const NetWebServiceRequest2ErrorDomain = @"NetWebServiceRequest2ErrorDomain";
@interface NetWebServiceRequest2 ()<ASIHTTPRequestDelegate>
@property (nonatomic, retain) __block ASIHTTPRequest* runningRequest;
@property (nonatomic, retain) NSRecursiveLock *cancelLock;

@end

@implementation NetWebServiceRequest2
...
+ (id)serviceRequestUrl:(NSString *)method
                 params:(NSDictionary *)params
                    tag:(NSInteger)tag {
    NSString *WebURL = @"http://webservice3819383118.wutongguo.com/RecruitmentService.asmx";
    ...
    NetWebServiceRequest2 *request = [[[self alloc] initWithUrl:WebURL SOAPActionURL:soapActionURL ServiceMethodName:method SoapMessage:soapMsg] autorelease];
    [request setTag:tag];
    return request;
}
...
- (void)NetWebServiceRequest2Started
{
    if (_delegate && [_delegate respondsToSelector:@selector(netRequestStarted2:)]) {
        [self.delegate netRequestStarted2:self];
    }
}

- (void)FinisheddidRecvedInfoToResult:(NSString *)result responseData:(GDataXMLDocument *)requestData
{
    if (_delegate && [_delegate respondsToSelector:@selector(netRequestFinished2: finishedInfoToResult: responseData:)]) {
		[_delegate netRequestFinished2:self finishedInfoToResult:result responseData:requestData];
	}
}
...
- (void) FaileddidRequestError:(int *)error
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(netRequestFailed2:didRequestError:)]) {
        [_delegate netRequestFailed2:self didRequestError:error];
    }
}
...
- (void)requestStarted:(ASIHTTPRequest *)request
{
    ...
    [self NetWebServiceRequest2Started];
}
