...
@interface FocusCampusViewController ()<NetWebServiceRequestDelegate, UITableViewDataSource, UITableViewDelegate, PopupViewDelegate>
...
@implementation FocusCampusViewController
...
- (void)getData {
    ...
    self.runningRequest = [NetWebServiceRequest serviceRequestUrl:@"GetPaAttentionByAppPaMainID" params:[NSDictionary dictionaryWithObjectsAndKeys:[CommonFunc getPaMainId], @"paMainID", [NSString stringWithFormat:@"%ld", (long)self.page], @"pageNo", @"4", @"attentionType", [CommonFunc getCode], @"code", nil] tag:1];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *content = [rowData objectForKey:@"CpBrandName"];
    float fontSize = 14;
    //宣讲会名称
    ...
    //宣讲会地点 地区学校
    content = [NSString stringWithFormat:@"[%@] %@", [rowData objectForKey:@"Abbr"], [rowData objectForKey:@"SchoolName"]];
    CustomLabel *lbSchool = [[CustomLabel alloc] initWithFixedHeight:CGRectMake(VIEW_X(lbDate), VIEW_BY(lbDate) + 5, widthForLabel, 20) content:content size:fontSize color:nil];
    ...
    //宣讲会地点 详细地址
    content = [rowData objectForKey:@"Address"];
    CustomLabel *lbAddress = [[CustomLabel alloc] initWithFixedHeight:CGRectMake(VIEW_BX(imgMap) + 1, VIEW_Y(lbSchool), widthForLabel - VIEW_BX(imgMap) + 20, 20) content:content size:fontSize color:TEXTGRAYCOLOR];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    companyCtrl.secondId = [rowData objectForKey:@"CpSecondID"];
...
- (void)favoriteClick:(UIButton *)sender {
    ...
    PopupView *alert = [[PopupView alloc] initWithWarningAlert:self.view title:@"提示" content:@"同学，你确定要取消关注该宣讲会吗？" okMsg:@"确定取消关注" cancelMsg:@"我点错啦"];
    ...
    if (request.tag == 1) {
        ...
        if (self.arrData.count == 0) {
            if (self.viewNoList == nil) {
                self.viewNoList = [[PopupView alloc] initWithNoListTips:self.tableView tipsMsg:@"<div style=\"text-align:center; font-size:14px;\"><p>同学，您尚未关注过任何宣讲会，去搜索并关注感兴趣的宣讲会，果儿会及时为您发送参会提醒！</p></div>"];
            }
